...
BEGIN
		...
		WITH ListData AS (
			...
			WHERE		
			 ANR.IsActive = 1
						--and AR.IsDeleted = 0
								--and	((@SearchText = N'')		OR (@SearchText = ANR.NormalizedName))	OR
		...
		--SET @AllUsersCount		 = (Select COUNT(*) from AspNetUsers where IsDeleted = 0);
		SET @AllUsersCount		 = (Select Count(Distinct AspNetUserRoles.UserId) from  AspNetUsers
												left join  AspNetUserRoles on AspNetUsers.Id = AspNetUserRoles.UserId
												full join AspNetRoles on AspNetUserRoles.RoleId = AspNetRoles.Id
												where AspNetUsers.IsDeleted = 0 and AspNetRoles.IsActive = 1);
