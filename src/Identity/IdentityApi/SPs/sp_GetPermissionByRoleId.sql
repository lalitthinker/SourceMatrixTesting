CREATE PROCEDURE [dbo].[sp_GetPermissionByRoleId]
(
...
BEGIN
		...
		WITH ListData AS (
			SELECT	DISTINCT	CC.Id							     As Id,												CT.Name						 As ClaimTypeName,												CG.Name                       AS ClaimGroupName,												CC.ClaimValue             As ClaimValue,												CC.ClaimTypeId           As ClaimTypeId,												CC.ClaimGroupId         As ClaimGroupId,												R.Name                           AS RoleName,												(select Count(*) from MappingRoles where (ApplicationRoleId = @ApplicationRoleId) AND [CustomClaimsId] = CC.Id)    AS IsAllowed,											(select Count(*) from MappingRoles where (ApplicationRoleId = @ApplicationRoleId) AND [CustomClaimsId] = CC.Id)    AS IsExpandable,											(select Count(*) from MappingRoles where (ApplicationRoleId = @ApplicationRoleId) AND [CustomClaimsId] = CC.Id)    AS PermissionSwitch								FROM	CustomClaims								As cc 			            left JOIN  MappingRoles      AS MR  ON CC.Id IN (Select mappingRoles.CustomClaimsId from mappingRoles  where mappingRoles.ApplicationRoleId= @ApplicationRoleId )						left JOIN ClaimTypes		     	As CT	 ON CT.Id = CC.ClaimTypeId						left JOIN ClaimGroups			As CG	 ON CG.Id = CC.ClaimGroupId						left Join AspNetRoles				AS R     ON R.Id  IN (Select mappingRoles.ApplicationRoleId from mappingRoles  where mappingRoles.ApplicationRoleId= @ApplicationRoleId )
					    
					       -- 	Filter using search text
			WHERE		  ((@SearchText = N'')          OR (CT.Name like Concat('%' ,@SearchText , '%'))
